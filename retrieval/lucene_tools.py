...
class Lucene(object):
    ...
    def get_document_fields(self, lucene_doc_id):
        if lucene_doc_id is None:
            print "Document is not found in the index."
            return None
        else:
            ...
            values = dict()
            for fname in fields:
                if fname == 'source' or fname == 'target':
                    if fname not in values:
                        values[fname] = []
                    for fv in doc.getValues(fname):
                        values[fname].append(fv)
            return values
    ...
    def get_lucene_query(self, query, field=FIELDNAME_CONTENTS):
        ...
        return QueryParser(Lucene.get_version(), field,
                           self.get_analyzer()).parse(QueryParser.escape(query))
    ...
    def score_query(self, query, field_content=FIELDNAME_CONTENTS, field_id=FIELDNAME_ID, num_docs=100):
        ...
        # add filter here: self.searcher.search(lucene_query, filter, num_docs).scoreDocs
        return self.__to_retrieval_results(scoredocs, field_id)

    def score_filter_query(self, query, source, field_content=FIELDNAME_CONTENTS, field_id=FIELDNAME_ID,
                           num_docs=100):
        """Scores a given query, filter base on the entity"""
        lucene_query = self.get_lucene_query(query, field_content)
        source_term = Term(self.FIELDNAME_SOURCE, source)
        target_term = Term(self.FIELDNAME_TARGET, source)
        source_filter = TermsFilter([source_term, target_term])
        # target_filter = TermsFilter([target_term])
        # filters = BooleanFilter()
        # filters.add(source_filter, BooleanClause.Occur.MUST)
        # filters.add(target_filter, BooleanClause.Occur.MUST)

        scoredocs = self.searcher.search(lucene_query, source_filter, num_docs).scoreDocs
