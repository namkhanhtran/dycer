...
class Retrieval(object):
    def __init__(self, config, lucene_vm_init=True):
        """
    ...
    def _first_pass_scoring(self, lucene, query, source=None):
        """
        ...
        if source is None:
            ...
            results = lucene.score_filter_query(query, source, field_content=self.config['first_pass_field'],
                                                field_id=self.config['field_id'],
                                                num_docs=self.config['first_pass_num_docs'])
    ...
    def _second_pass_scoring(self, res1, scorer):
        ...
        for doc_id, orig_score in res1.get_scores_sorted():
            ...
            score = math.exp(scorer.score_doc(doc_id, doc_id_int))

            field_values = self.lucene.get_document_fields(doc_id_int)
            source = field_values['source'][0]
            target = field_values['target'][0]
            results.append((source, target), score)
            # results.append((doc_id, doc_id_int), score)
        # print "done"
        return results

    def query(self, query, source=None):
        query = Lucene.preprocess(query)
        if query == '':
            return None
        res1 = self._first_pass_scoring(self.lucene, query, source)
        # res2 = self._first_pass_scoring(self.lucene, query, target, source)
        # print "scoring: " + query
        # first pass scoring
        scorer = Scorer.get_scorer(self.config['model'], self.lucene, query, self.config)
        results1 = self._second_pass_scoring(res1, scorer)
        # results2 = self._second_pass_scoring(res2, scorer)
        return results1
    ...
    def retrieve(self):
        ...
        # apply filter query here
        # sourceQuery = TermQuery(Term('source', '44849'))
        # f = QueryWrapperFilter(sourceQuery)
        # modify score_query with filter
